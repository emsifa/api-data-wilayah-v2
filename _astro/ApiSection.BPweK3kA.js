...
}`},{method:`GET`,path:`/provinces.json`,description:`Ambil semua provinsi — lengkap sama kapital, koordinat, populasi & luasnya (level 1)`,curl:`curl ${L}/provinces.json`,snippets:{curl:`curl ${L}/provinces.json`,fetch:`const res = await fetch("${L}/provinces.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/provinces.json"
);`,laravel:`$response = Http::get("${L}/provinces.json");
$provinces = $response->json("data");`,go:`resp, err := http.Get("${L}/provinces.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/provinces.json")
provinces = r.json()["data"]`,php:`$ch = curl_init("${L}/provinces.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$provinces = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/provinces.json"));
...
}`},{method:`GET`,path:`/provinces/{code}.json`,description:`Kepoin satu provinsi aja by kode — misal 32 = Jawa Barat (level 1)`,curl:`curl ${L}/provinces/32.json`,snippets:{curl:`curl ${L}/provinces/32.json`,fetch:`const res = await fetch("${L}/provinces/32.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/provinces/32.json"
);`,laravel:`$response = Http::get("${L}/provinces/32.json");
$province = $response->json("data");`,go:`resp, err := http.Get("${L}/provinces/32.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/provinces/32.json")
province = r.json()["data"]`,php:`$ch = curl_init("${L}/provinces/32.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$province = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/provinces/32.json"));
...
}`},{method:`GET`,path:`/regencies/{province_id}.json`,description:`Daftar kab/kota di provinsi tertentu — misal semua kota di Jawa Barat (level 2)`,curl:`curl ${L}/regencies/32.json`,snippets:{curl:`curl ${L}/regencies/32.json`,fetch:`const res = await fetch("${L}/regencies/32.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/regencies/32.json"
);`,laravel:`$response = Http::get("${L}/regencies/32.json");
$regencies = $response->json("data");`,go:`resp, err := http.Get("${L}/regencies/32.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/regencies/32.json")
regencies = r.json()["data"]`,php:`$ch = curl_init("${L}/regencies/32.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$regencies = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/regencies/32.json"));
...
}`},{method:`GET`,path:`/regencies/{regency_id}.json`,description:`Detail satu kab/kota by kode — bonus info provinsinya juga (level 2)`,curl:`curl ${L}/regencies/32.73.json`,snippets:{curl:`curl ${L}/regencies/32.73.json`,fetch:`const res = await fetch("${L}/regencies/32.73.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/regencies/32.73.json"
);`,laravel:`$response = Http::get("${L}/regencies/32.73.json");
$regency = $response->json("data");`,go:`resp, err := http.Get("${L}/regencies/32.73.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/regencies/32.73.json")
regency = r.json()["data"]`,php:`$ch = curl_init("${L}/regencies/32.73.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$regency = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/regencies/32.73.json"));
...
}`},{method:`GET`,path:`/districts/{regency_id}.json`,description:`Daftar kecamatan di kab/kota tertentu — lengkap dengan koordinat lat/lng (level 3)`,curl:`curl ${L}/districts/32.73.json`,snippets:{curl:`curl ${L}/districts/32.73.json`,fetch:`const res = await fetch("${L}/districts/32.73.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/districts/32.73.json"
);`,laravel:`$response = Http::get("${L}/districts/32.73.json");
$districts = $response->json("data");`,go:`resp, err := http.Get("${L}/districts/32.73.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/districts/32.73.json")
districts = r.json()["data"]`,php:`$ch = curl_init("${L}/districts/32.73.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$districts = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/districts/32.73.json"));
...
}`},{method:`GET`,path:`/districts/{district_id}.json`,description:`Detail satu kecamatan — plus tau dia dari provinsi & kab/kota mana, lengkap dengan koordinat (level 3)`,curl:`curl ${L}/districts/32.73.01.json`,snippets:{curl:`curl ${L}/districts/32.73.01.json`,fetch:`const res = await fetch("${L}/districts/32.73.01.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/districts/32.73.01.json"
);`,laravel:`$response = Http::get("${L}/districts/32.73.01.json");
$district = $response->json("data");`,go:`resp, err := http.Get("${L}/districts/32.73.01.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/districts/32.73.01.json")
district = r.json()["data"]`,php:`$ch = curl_init("${L}/districts/32.73.01.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$district = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/districts/32.73.01.json"));
...
}`},{method:`GET`,path:`/villages/{district_id}.json`,description:`Daftar kelurahan/desa di kecamatan itu — udah include kode pos dan koordinat (level 4)`,curl:`curl ${L}/villages/32.73.01.json`,snippets:{curl:`curl ${L}/villages/32.73.01.json`,fetch:`const res = await fetch("${L}/villages/32.73.01.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/villages/32.73.01.json"
);`,laravel:`$response = Http::get("${L}/villages/32.73.01.json");
$villages = $response->json("data");`,go:`resp, err := http.Get("${L}/villages/32.73.01.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/villages/32.73.01.json")
villages = r.json()["data"]`,php:`$ch = curl_init("${L}/villages/32.73.01.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$villages = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/villages/32.73.01.json"));
...
}`},{method:`GET`,path:`/villages/{village_id}.json`,description:`Detail satu kelurahan/desa — lengkap kode pos, koordinat + provinsi, kab/kota, kecamatan (level 4)`,curl:`curl ${L}/villages/32.73.01.1001.json`,snippets:{curl:`curl ${L}/villages/32.73.01.1001.json`,fetch:`const res = await fetch("${L}/villages/32.73.01.1001.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/villages/32.73.01.1001.json"
);`,laravel:`$response = Http::get("${L}/villages/32.73.01.1001.json");
$village = $response->json("data");`,go:`resp, err := http.Get("${L}/villages/32.73.01.1001.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/villages/32.73.01.1001.json")
village = r.json()["data"]`,php:`$ch = curl_init("${L}/villages/32.73.01.1001.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$village = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/villages/32.73.01.1001.json"));
...
}`},{method:`GET`,path:`/postal-codes/{postal_code}.json`,description:`Cari kelurahan by kode pos — misal 40152 tuh daerah mana aja (level 4)`,curl:`curl ${L}/postal-codes/40152.json`,snippets:{curl:`curl ${L}/postal-codes/40152.json`,fetch:`const res = await fetch("${L}/postal-codes/40152.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/postal-codes/40152.json"
);`,laravel:`$response = Http::get("${L}/postal-codes/40152.json");
$villages = $response->json("data");`,go:`resp, err := http.Get("${L}/postal-codes/40152.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/postal-codes/40152.json")
villages = r.json()["data"]`,php:`$ch = curl_init("${L}/postal-codes/40152.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$villages = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/postal-codes/40152.json"));
final villages = jsonDecode(res.body)["data"] as List;`},response:`{
  "data": [
    {
      "id": "32.73.01.1001",
...
}`},{method:`GET`,path:`/paths/{region_id}.json`,description:`Butuh polygon buat peta? Ambil garis batas wilayah di sini — support semua level: provinsi, kab/kota, kecamatan, kelurahan (compact JSON)`,curl:`curl ${L}/paths/32.json`,snippets:{curl:`curl ${L}/paths/32.json`,fetch:`const res = await fetch("${L}/paths/32.json");
const { data, meta } = await res.json();`,axios:`const { data: { data, meta } } = await axios.get(
  "${L}/paths/32.json"
);`,laravel:`$response = Http::get("${L}/paths/32.json");
$path = $response->json("data");`,go:`resp, err := http.Get("${L}/paths/32.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/paths/32.json")
path = r.json()["data"]`,php:`$ch = curl_init("${L}/paths/32.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$path = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/paths/32.json"));
...
}`},{method:`GET`,path:`/missings.json`,description:`Cek yang belum lengkap — list wilayah dimana has_path==false OR has_latlng==false (flat + summary by_level)`,curl:`curl ${L}/missings.json`,snippets:{curl:`curl ${L}/missings.json`,fetch:`const res = await fetch("${L}/missings.json");
const { data, meta, summary } = await res.json();`,axios:`const { data: { data, meta, summary } } = await axios.get(
  "${L}/missings.json"
);`,laravel:`$response = Http::get("${L}/missings.json");
$missings = $response->json("data");`,go:`resp, err := http.Get("${L}/missings.json")
if err != nil { /* handle */ }
defer resp.Body.Close()
var result map[string]any
json.NewDecoder(resp.Body).Decode(&result)`,python:`import requests

r = requests.get("${L}/missings.json")
missings = r.json()["data"]`,php:`$ch = curl_init("${L}/missings.json");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$body = json_decode(curl_exec($ch), true);
curl_close($ch);
$missings = $body["data"];`,dart:`final res = await http.get(Uri.parse("${L}/missings.json"));
...
}`}],z=[{key:`curl`,label:`curl`},{key:`fetch`,label:`Fetch`},{key:`axios`,label:`Axios`},{key:`laravel`,label:`Laravel`},{key:`go`,label:`Go`},{key:`python`,label:`Python`},{key:`php`,label:`PHP`},{key:`dart`,label:`Dart`}];function B(){let[e,t]=(0,u.useState)(null),[n,r]=(0,u.useState)(`curl`),i=async(e,n)=>{await navigator.clipboard.writeText(e),t(n),setTimeout(()=>t(null),2e3)},l=`https://www.emsifa.com/api-data-wilayah-v2/v2`;return(0,d.jsx)(`section`,{id:`api`,className:`relative z-10 overflow-hidden rounded-t-[32px] border-t border-slate-200 bg-white shadow-[0_-12px_40px_rgba(0,0,0,0.12)]`,children:(0,d.jsxs)(`div`,{className:`mx-auto max-w-6xl px-6 py-10 md:px-8 md:py-14`,children:[(0,d.jsxs)(`div`,{className:`flex flex-col gap-6 md:flex-row md:items-end md:justify-between`,children:[(0,d.jsxs)(`div`,{children:[(0,d.jsxs)(`div`,{className:`inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold tracking-widest text-slate-600 uppercase`,children:[(0,d.jsx)(`span`,{className:`h-1.5 w-1.5 rounded-full bg-emerald-500`}),`API Statis`]}),(0,d.jsx)(`h2`,{className:`mt-3 text-3xl font-extrabold tracking-tight text-slate-900 md:text-4xl`,children:`Tinggal fetch, beres`}),(0,d.jsxs)(`p`,{className:`mt-2 max-w-xl text-sm leading-relaxed text-slate-600`,children:[`Nggak perlu bikin backend. Datanya cuma file JSON di GitHub Pages — tinggal `,(0,d.jsx)(`code`,{className:`rounded bg-slate-100 px-1 py-0.5 font-mono text-xs`,children:`fetch`}),` aja. Tanpa API key, tanpa rate limit. Gas langsung pakai.`]})]}),(0,d.jsxs)(`div`,{className:`flex flex-col gap-3`,children:[(0,d.jsxs)(`div`,{className:`flex items-center gap-2`,children:[(0,d.jsx)(`code`,{className:`rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 font-mono text-xs text-slate-700 md:text-sm`,children:l}),(0,d.jsx)(`button`,{onClick:()=>i(l,`base`),className:`inline-flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 cursor-pointer`,"aria-label":`Copy base URL`,children:e===`base`?(0,d.jsx)(a,{size:16,className:`text-emerald-600`}):(0,d.jsx)(o,{size:16})})]}),(0,d.jsxs)(`div`,{className:`flex flex-wrap gap-2`,children:[(0,d.jsxs)(`a`,{href:`/api-data-wilayah-v2/wilayah-postman.json`,download:!0,className:`inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50`,children:[(0,d.jsx)(s,{size:14}),` Postman`]}),(0,d.jsxs)(`a`,{href:`/api-data-wilayah-v2/openapi.yml`,download:!0,className:`inline-flex items-center gap-1.5 rounded-xl border border-slate-900 bg-slate-900 px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-slate-800`,children:[(0,d.jsx)(s,{size:14}),` openapi.yml`]})]})]})]}),(0,d.jsx)(`div`,{className:`mt-8 grid gap-3`,children:R.map(t=>(0,d.jsx)(f,{title:t.path,subtitle:t.description,badge:(0,d.jsx)(`span`,{className:`shrink-0 rounded-full bg-emerald-500 px-2.5 py-1 text-[11px] font-bold tracking-widest text-white uppercase`,children:`GET`}),children:(0,d.jsxs)(`div`,{className:`space-y-4`,children:[(0,d.jsxs)(`div`,{children:[(0,d.jsxs)(`div`,{className:`mb-2 flex flex-wrap items-center gap-2`,children:[(0,d.jsx)(`span`,{className:`text-xs font-semibold tracking-widest text-slate-500 uppercase`,children:`Request`}),(0,d.jsx)(`div`,{className:`hidden md:flex items-center gap-0.5 rounded-xl bg-slate-100 p-1`,children:z.map(e=>(0,d.jsx)(`button`,{onClick:()=>r(e.key),className:`rounded-lg px-2.5 py-1 text-xs font-semibold transition cursor-pointer ${n===e.key?`bg-white text-slate-900 shadow-sm`:`text-slate-500 hover:text-slate-700`}`,children:e.label},e.key))}),(0,d.jsx)(`select`,{value:n,onChange:e=>r(e.target.value),className:`md:hidden rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-medium text-slate-700 cursor-pointer`,children:z.map(e=>(0,d.jsx)(`option`,{value:e.key,children:e.label},e.key))}),(0,d.jsxs)(`button`,{onClick:()=>i(t.snippets[n],t.path+`-code`),className:`ml-auto inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50 cursor-pointer`,children:[e===t.path+`-code`?(0,d.jsx)(a,{size:12,className:`text-emerald-600`}):(0,d.jsx)(o,{size:12}),`Copy `,z.find(e=>e.key===n)?.label]})]}),(0,d.jsx)(I,{code:t.snippets[n],lang:n})]}),(0,d.jsxs)(`div`,{children:[(0,d.jsxs)(`div`,{className:`mb-2 flex items-center justify-between`,children:[(0,d.jsx)(`span`,{className:`text-xs font-semibold tracking-widest text-slate-500 uppercase`,children:`Response`}),(0,d.jsxs)(`a`,{href:t.curl.replace(`curl `,``),target:`_blank`,rel:`noreferrer`,className:`inline-flex items-center gap-1 text-xs font-medium text-slate-600 hover:text-slate-900`,children:[`Try `,(0,d.jsx)(c,{size:12})]})]}),(0,d.jsx)(v,{code:t.response,theme:`light`})]})]})},t.path))}),(0,d.jsxs)(`p`,{className:`mt-6 text-center text-xs text-slate-400`,children:[`Ada `,R.length,` endpoint total • Semuanya balik`,` `,(0,d.jsx)(`code`,{className:`rounded bg-slate-100 px-1 py-0.5 font-mono`,children:`{ data, meta }`}),` `,`• Udah ke-cache CDN GitHub Pages, ngebut`]})]})})}export{B as ApiSection};